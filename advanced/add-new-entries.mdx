---
title: "Add New BibTeX Entries from a DOI, ISBN, or URL Link"
sidebarTitle: "Add New Entries"
description: "Select a DOI, ISBN, or URL containing a DOI, then use Alfred's Universal Action or a hotkey to fetch metadata and append it to your BibTeX library."
---
...
4. **Metadata is fetched** from an online API:
   - **DOI** → [CrossCite API](https://citation.crosscite.org/docs.html) at `https://doi.org/` with `Accept: application/vnd.citationstyles.csl+json` header, returning CSL-JSON
   - **ISBN** → [OpenLibrary API](https://openlibrary.org/developers/api) first (`openlibrary.org/api/books`); falls back to [Google Books API](https://developers.google.com/books/docs/v1/using) if OpenLibrary has no data
